...
卡片上的复制按钮支持单击复制并保持窗口打开，点击正文仍用于阅读；标签与分组通过右键管理。窗口保留原生红绿灯，设置与备份使用资料柜统一的深浅风格。

继续使用 SwiftUI + AppKit + CoreData；深浅外观通过原生磨砂融入桌面。点击结果只阅读，“复制”保留窗口，“复制并收起”返回之前的 App。⌘⇧C 唤出，⌘K 搜索，方向键选择，回车复制收起；编辑时 ⌘S 保存、⌘↵ 保存并复制收起。原有 JSON 导入、剪贴板保留设置、备份与恢复继续可用。旧分类升级为标签，备份 v2 包含多标签、分组、无标签片段及图片，并可导入 v1。
