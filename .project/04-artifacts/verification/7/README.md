...
**正式新版已在用户授权下接替旧版并迁移真实数据。** 最新事实见 [正式迁移与重启验证](production-migration.md)；下方早期“未安装 / 未升级真实库”的说明仅描述各轮历史状态。

最新侧栏排版见 [对齐前后截图与验证](sidebar-alignment.md)。本轮只改视图布局，窗口层级、材质与复制规则沿用前轮。

最新窗口层级、来源图标与统一材质见 [窗口与材质验证](window-and-material.md)，保留上一轮单击查看、双击复制规则。

**当前交互与输入证据见 [单击查看、双击复制与中文搜索](input-and-interaction.md)。** 它替代下方历史记录中的单击复制规则；真实拼音上屏已完成基础路径验证，其他输入法及复杂组合操作仍未全面验收。

后续体验修订见 [设置风格、红绿灯与卡片复制验证](feedback.md)。下文保留首轮代码与截图，不代表后续修订后的最新外观。

此前整卡复制规则与卡片边界见 [整卡复制验证](whole-card.md)，保留为历史证据。

中性灰绿主题与查看按钮移除见 [主题验证](neutral-green.md)。历史截图按各自提交保留，当前排版以本页顶部的最新记录为准。
