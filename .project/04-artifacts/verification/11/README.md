...
关联 [Issue #11](https://github.com/airsulG/cheatsheet/issues/11)。本轮实施从 main `883ae7dca6c7bc4a96e1264964513527a3163479` 开始，分支 `codex/sidebar-width-tag-pins`。2026-09-15 完成代码、隔离功能检查和本地构建；交付为独立 Draft PR，尚未合并或替换日常 App。
...
原生事件检查证明中间宽度会更新、达到边界后停止，不等待 mouseUp 才改变布局；代码中的拖动回调不包含查询、保存、计时器或追赶动画。新增的布局成本是侧栏宽度变化引起的网格换列，静止时没有持续任务。尚未取得实际拖动的逐帧录像或 FPS 测量，因此不宣称体验流畅度验收已通过；PR 保留这个明确缺口供人工体验确认。用户日常数据库迁移、安装和合并属于后续交付决定。
