...
- 当前阶段：方案已确认，等待开始编码。
- 当前分支：`main`。
- 当前是否允许自动执行：允许创建和维护本任务文档；功能代码修改需要用户再次确认“开始实现”。
- 当前阻塞：还没有开始写功能代码；需要确认第一版是否包含“定时导出”还是先只做“手动导出 / 手动导入”。
- 下一步：用户确认开始实现后，先新增备份数据结构和备份服务，再接入设置界面。
...
- 已确认内容：
  ...
  - 用户要求创建本任务的自循环文档。
- 未确认内容：
  - 定时导出的周期：启动时、每天、每周，还是用户自定义间隔。
  - 导入冲突处理方式：追加导入、按同名分类合并、还是先清空再恢复。
  - 备份文件是否需要保留多份，还是每次覆盖同一个文件。
...
- 风险和边界：
  - 导入可能造成重复数据，需要确认冲突处理方式。
  - 定时导出需要避免在用户正在编辑时写出半完成数据。
  - 选择文件夹后，macOS 沙盒权限可能需要安全书签；如果当前项目没有开启沙盒，可先用普通路径保存，后续再补安全书签。
